...
class MultiClassDiceLoss(nn.Module):
    ...
    def forward(self, logits, targets):
        """
        logits: 网络的原始输出 [B, 16, H, W] (没有经过 Softmax)
        targets: 真实标签 [B, H, W] (0-15 的整数)
        """
        # 1. 对 Logits 做 Softmax，得到 0~1 的概率分布
        probs = F.softmax(logits, dim=1)

        # 2. 将真实的整数标签转为 One-Hot 格式
        # F.one_hot 输出形状是 [B, H, W, 16]，需要 permute 变成 [B, 16, H, W]
