...
class Visualizer():
    def __init__(self, opt):
        ...
        self.name = opt.name
        self.tf_log_dir = os.path.join(opt.checkpoints_dir, opt.name, 'tf_logs')
        self.writer = SummaryWriter(self.tf_log_dir)
        if self.tf_log:
    ...
    def display_current_results(self, visuals, epoch, step):
        ...
        for label, image in visuals.items():
            # 注意：需要确保 image 是 Tensor 格式且范围在 [0, 1]
            normalized_array = image.astype(np.float32) / 255.0
            if normalized_array.ndim == 4:
                # 从 (1, 256, 256, 4) 变成 (256, 256, 4)
                normalized_array = normalized_array[0]
            self.writer.add_image(f'Visuals/{label}', normalized_array, epoch, dataformats='HWC')

    def plot_current_losses(self, losses, step):
        for label, value in losses.items():
            self.writer.add_scalar(f'Loss/{label}', value, step)
    ...
    def convert_visuals_to_numpy(self, visuals):
        for key, t in visuals.items():
            ...
            if 'input_label' == key or 'real_image' == key:
